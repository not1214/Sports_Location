  ...
  <div class="row">
    ...
    <div class="col-10 offset-1 col-md-8 offset-md-0">
      ...
      <div class="row mb-3">
        ...
        <img class="col-8 offset-2 col-md-5 col-lg-3 offset-md-0 me-2 img-fluid" src="{{ asset($reservation->event->event_image) }}" style="border-radius:30%;max-height:120px;">
        @else
        ...
        <div class="col-10 offset-1 col-md-12 col-lg-8 offset-md-0">
          <div class="col-12">
            <a href="{{ route('admin.events.show', ['event'=>$reservation->event->id]) }}" class="fs-3 fw-bold">{{ Str::limit($reservation->event->title, 30) }}</a>
          </div>

          <div class="col-12">
            @if (!empty($reservation->event->user->username))
            開催者：<span><a href="{{ route('admin.user.show', ['username'=>$reservation->event->user->username]) }}">{{ $reservation->event->user->username }}</a></span>
            @else
